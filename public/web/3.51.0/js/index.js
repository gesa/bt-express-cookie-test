...
var destroyFocusIntercept = focusIntercept.destroy;
var matchFocusElement = focusIntercept.matchFocusElement;
...
function HostedFields(options) {
  ...
  Object.keys(options.fields).forEach(function (key) {
    ...
    this._injectedNodes.push.apply(this._injectedNodes, injectFrame(frame, container, function () {
      ...
      console.log('TRIGGER_INPUT_FOCUS', 'internal', key);
    }));
  ...
  function checkForUserFocusableElement(element) {
  ...
  function removeExtraFocusElements(element) {
    if (checkForUserFocusableElement(element)) {
      if (element.getAttribute('id') && matchFocusElement(element.getAttribute('id'))) {
        self._bus.emit(events.REMOVE_FOCUS_INTERCEPTS, element.getAttribute('id'));
        console.log('REMOVE_FOCUS_INTERCEPTS', 'external', element.id, 'during setup loop')
      }
  ...
  this._bus.on(events.TRIGGER_FOCUS_CHANGE, function (type, direction) {
    var currentIndex, targetElement, checkoutForm, navigationHelpers, nav;
    ...
    if (document.forms.length < 1 || typeof checkoutForm === 'undefined') {
      ...
      console.log('REMOVE_FOCUS_INTERCEPTS', 'external', 'form length 0')
    ...
    navigationHelpers = {
      // NEXT_MAJOR_VERSION these keys (`before` & `after`) should be the constants
      before: {
      ...
      after: {
        checkIndexBounds: function (index) {
          return index > checkoutForm.length - 1;
        },
        indexChange: 1
      }
    };
    nav = navigationHelpers[direction];
    currentIndex = checkoutForm.indexOf(sourceElement);

    do {
      currentIndex += nav.indexChange;
      if (nav.checkIndexBounds(currentIndex)) { return; }
      targetElement = checkoutForm[currentIndex];
    } while (!checkForUserFocusableElement(targetElement));

    if (matchFocusElement(targetElement.getAttribute('id'))) {
      self._bus.emit(events.TRIGGER_INPUT_FOCUS, targetElement.getAttribute('data-braintree-type'));
      console.log('TRIGGER_INPUT_FOCUS', 'external', targetElement.getAttribute('data-braintree-type'), 'focus change handler');
    } else {
      targetElement.focus();
      console.log('focus() DOM event', 'external', targetElement, 'focus change handler');
    }
  });
  ...
  Promise.all(frameReadyPromises).then(function (results) {
    var reply = results[0];
    var elementsIndex, checkoutForm;
    ...
    if (document.forms.length < 1) {
      ...
      for (elementsIndex = 0; elementsIndex < checkoutForm.length; elementsIndex++) {
        if (removeExtraFocusElements(checkoutForm[elementsIndex])) { break; }
      }
...
HostedFields.prototype._setupLabelFocus = function (type, container) {
  ...
  function triggerFocus() {
    ...
    console.log('TRIGGER_INPUT_FOCUS', 'external', type, 'function triggerFocus()')
  }
...
HostedFields.prototype.focus = function (field) {
  ...
  if (!allowedFields.hasOwnProperty(field)) {
    ...
    console.log('TRIGGER_INPUT_FOCUS', field, 'external', 'prototype focus method')
  }
...
var focusIntercept = {
  ...
  destroy: function (idString) {
    ...
    if (focusInputs.length > 0) {
      focusInputs.forEach(function (node) {
        if (node !== null && node.nodeType === 1 && focusIntercept.matchFocusElement(node.getAttribute('id'))) {
  ...
  matchFocusElement: function (idString) {
    var idComponents = idString.split('-');

    if (idComponents.length !== 3) { return false; }

    return Boolean(
      idComponents[0].match('bt') &&
      allowedFields.indexOf(idComponents[1]) > -1 &&
      (idComponents[2].match(directions.BACK) || idComponents[2].match(directions.FORWARD))
    );
  }
};
...
},{"./browser-detection":81,"./constants":82,"@braintree/class-list":18}],86:[function(_dereq_,module,exports){
'use strict';
